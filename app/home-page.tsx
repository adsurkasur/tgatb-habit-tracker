...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handler = App.addListener('backButton', () => {
      ...
      if (now - lastBackPressRef.current < 2000) {
        ...
        toast({ title: t("toasts.pressBackAgainExit"), duration: 3000 });
      }
    });
  ...
  const deletedHabit = deleteHabit({ id: habitId });
    ...
    if (deletedHabit) {
      ...
      toast({
        title: t("toasts.habitDeletedTitle"),
        description: t("toasts.habitDeletedDescription"),
        action: (
          <ToastAction
            altText={t("toasts.undoDeleteAltText")}
                  ...
                  <>
                    <div className="flex justify-center mt-4">
                      <div className="text-xs text-muted-foreground text-center">
                        {t("counter", {
                          current: currentHabitIndex + 1,
                          total: goodHabits.length + badHabits.length,
                        })}
